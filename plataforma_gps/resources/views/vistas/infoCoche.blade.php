...
<head>
    ...
    <title>Detalle: {{ $device['name'] ?? 'Vehículo' }}</title>
...
<body class="bg-gray-900 min-h-screen text-gray-100 p-6 font-sans">
    ...
    <div class="max-w-6xl mx-auto mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
                <h1 class="text-3xl font-bold text-white leading-none">{{ $device['name'] ?? 'Desconocido' }}</h1>
                <p class="text-xs text-gray-400 mt-1">ID: {{ $device['uniqueId'] ?? 'N/A' }}</p>
            </div>
    ...
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <span class="text-4xl font-mono font-bold text-{{ $device['salud_bateria']['color'] ?? 'gray' }}-400">
                            {{ $device['posicion']['attributes']['power'] ?? ($device['posicion']['attributes']['batteryLevel'] ?? 0) }}<span class="text-lg">V</span>
                        </span>
                        <p class="text-sm text-gray-400 mt-1">Batería del Vehículo</p>
                    </div>
                    <div class="h-14 w-14 rounded-full bg-{{ $device['salud_bateria']['color'] ?? 'gray' }}-900/30 flex items-center justify-center border border-{{ $device['salud_bateria']['color'] ?? 'gray' }}-500/50">
                        <svg class="w-7 h-7 text-{{ $device['salud_bateria']['color'] ?? 'gray' }}-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                    </div>
                </div>
                <div class="mt-4 bg-gray-700 rounded-full h-1.5 w-full overflow-hidden">
                    <div class="h-full bg-{{ $device['salud_bateria']['color'] ?? 'gray' }}-500" style="width: 100%"></div>
                </div>
                <p class="text-xs text-right mt-2 text-{{ $device['salud_bateria']['color'] ?? 'gray' }}-300 font-bold">
                    {{ $device['salud_bateria']['estado'] ?? 'Sin Datos' }}
                </p>
            </div>
        ...
        <div class="lg:col-span-2 bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden flex flex-col h-[500px] lg:h-auto">
            <div id="mapDetail" class="flex-1 w-full h-full min-h-[400px]"></div>
    ...
    <script>
        ...
        if (lat !== 0 && lng !== 0) {
            ...
            // Usamos OSM estándar para que se vea claro, o puedes cambiarlo si prefieres oscuro
            ...
            document.getElementById('mapDetail').innerHTML = '<div class="h-full flex items-center justify-center text-gray-500">Sin coordenadas GPS válidas</div>';
        }
    </script>
    @else
    <script>
         document.getElementById('mapDetail').innerHTML = '<div class="h-full flex items-center justify-center text-gray-500">Esperando primer reporte GPS...</div>';
    </script>
    @endif
</body>
</html>